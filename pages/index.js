...
const index = () => {
	...
	const [clCourses, setCLCourses] = useState('~')
	const [ctpCourses, setctpCourses] = useState('~')
	const [mCourses, setMCourses] = useState('~')

	useEffect(async () => {
		setAPCourses((await cor()).length)
		setCLCourses((await cor(2)).length)
		setctpCourses((await cor(3)).length)
		setMCourses((await cor(4)).length)
	}, [])
	return (
		<Layout waves header>
			<Box width={[1, 1 / 2, 2 / 3]} m={1} p={2} mx="auto">
				...
				>
					...
					>
						<Item num={apCourses} text="AP®️ Courses"/>
						<Item num={clCourses} text="College-Level Courses"/>
						<Item num={ctpCourses} text="College & Test Prep Courses"/>
						<Item num={mCourses} text="Misc. Courses"/>
					</Box>
			...
			<Box mt={3} mb={6}>
				<Text textAlign="center" fontSize={[3, 4, 5]}>
					...
					{apCourses + ctpCourses + mCourses + clCourses} courses.
				</Text>
				<Text textAlign="center" fontSize={[3, 4, 5]}>
